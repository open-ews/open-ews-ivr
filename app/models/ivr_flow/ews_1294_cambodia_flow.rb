...
module IVRFlow
  class EWS1294CambodiaFlow
    ...
    class FeedbackMenu < IVRFlow::Menu
      PROMPTS = [
        :feedback_missing_address_prompt,
        :feedback_registration_issue_prompt,
        :feedback_no_alerts_received_prompt,
        :feedback_content_comprehension_difficulty_prompt
      ].freeze

      def valid_choice?
        (1..PROMPTS.size).member?(response.choice)
      end
    ...
    def call
      ...
      when "feedback_menu_prompted"
        if feedback_menu.valid_choice?
          record_feedback(prompt: feedback_menu.selection)
        elsif feedback_menu.response.start_over?
          prompt_main_menu
        else
          prompt_feedback_menu
        end
    ...
    def record_feedback(prompt:)
      twiml_builder.record(
        before: -> { it.play(url: build_audio_url(filename: prompt, language: "khm", file_extension: "mp3")) },
